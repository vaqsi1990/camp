...
import { profileSchema, validateWithZod, imageSchema, propertySchema } from "./schemas"
import db from './db';
...
export const createProfileAction =  async( prevState:any, formData:FormData) => {
   try {
      const user = await currentUser();
      if (!user) throw new Error('no user here, go log in');
      ...
      console.log(error);
      return { message: error instanceof Error ? error.message : 'Wrong, try again' };
   }
...
export const updateProfileImageAction = async (
  ...
  try {
    ...
    const validatedFields = validateWithZod(imageSchema, { image });
